...
for teamname in allteam:
    ...
    if not os.path.exists('./Teams/'+teamname):
        os.mkdir('./Teams/'+teamname)
 
    line = fp1.readline()  # 调用文件的 readline()方法

    head=['日期','对手','得分','时间','命中%','投篮','三分%','三分','罚球%','罚球','篮板','前场','后场','助攻','抢断','盖帽','犯规']

    while line:
        print(line)
        bro.get(line)  

        biaoge = bro.find_element_by_xpath('//*[@id="tab-grid"]/span')
        biaoge.click()

        jiqian = bro.find_elements_by_xpath('//*[@id="main-wrap"]/div[2]/div[2]/div[2]/ul[1]/li[1]/a')[0]
        jiqian.click()
        
 
        page_text = bro.page_source
        tree = etree.HTML(page_text)
        flag = tree.xpath('//*[@id="ct-grid"]/a/@style')
        if flag[0] == 'block' or len(flag)==0:
            loadMore = bro.find_element_by_xpath('//*[@id="ct-grid"]/a')
            loadMore.click()

        page_text = bro.page_source
        tree = etree.HTML(page_text)
        name=tree.xpath('//*[@id="inforCard"]/div[1]/h1/text()')[0]
        print('./Teams/'+teamname+'/'+name+'.csv')
        newFile = open('./Teams/'+teamname+'/'+name+'.csv', 'w')
        newWriter = csv.writer(newFile, lineterminator='\n')
        newWriter.writerow(['季前赛'])
        newWriter.writerow(head)

        trList = tree.xpath('//*[@id="ct-grid"]/div/table/tbody/tr')
        ...
        changgui=bro.find_elements_by_xpath('//*[@id="main-wrap"]/div[2]/div[2]/div[2]/ul[1]/li[2]/a')[0]
        changgui.click()

        page_text = bro.page_source
        tree = etree.HTML(page_text)
        flag = tree.xpath('//*[@id="ct-grid"]/a/@style')[0]
        if flag == 'block':
            loadMore = bro.find_element_by_xpath('//*[@id="ct-grid"]/a')
            loadMore.click()
            print('click')
        ...
        jihou=bro.find_elements_by_xpath('//*[@id="main-wrap"]/div[2]/div[2]/div[2]/ul[1]/li[3]/a')[0]
        jihou.click()
